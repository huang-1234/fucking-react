...
class AsyncController {
  ...
  constructor(concurrency, minRequests = 1) {
    ...
    /**
     * @desc 结果队列
     * @type {Array<{idx: number, result: T} | {idx: number, error: Error}>}
     */
    this.results = [];
    /**
     * @desc 正在进行的任务数
  ...
  /**
   * 运行任务
   * @param {Array<() => Promise<T>>} tasks 任务数组
   * @returns {Promise<Array<{idx: number, result: T} | {idx: number, error: Error}>>} 结果
   */
  async run(tasks) {
   ...
   * @returns {Promise<{idx: number, result: T} | {idx: number, error: Error}>} 结果
   */
  _enqueue(task, idx) {
