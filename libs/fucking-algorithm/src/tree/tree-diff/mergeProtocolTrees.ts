...
export function mergeProtocolTrees(treeOrigin: TreeData, treeIncrement: TreeData): MergeResult {
  const result = { ...treeOrigin };
  const diffMap = new Map();

  // 1. 遍历树B记录差异节点
  const traverseAndDiff = (node: TreeNode, path: string[] = []) => {
