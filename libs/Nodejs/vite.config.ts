...
// 动态导入MDX相关模块
const mdxPromise = import('@mdx-js/rollup');
...
export default async function () {
  // 等待动态导入完成
  const mdx = (await mdxPromise).default;
  const remarkGfm = (await remarkGfmPromise).default;
  const rehypeHighlight = (await rehypeHighlightPromise).default;
