...
import sseMiddleware from './sse-middleware-fixed.js';
...
async function createServer() {
  ...
  // Use Vite's connect middleware - 放在SSR处理之前
  app.use(async (ctx, next) => {
  ...
  app.use(async (ctx) => {
    ...
    if (ctx.path.includes('/api/events') || ctx.path.includes('/api/stream') ||
        ctx.headers.accept === 'text/event-stream' || ctx.status !== 404) {
      return;
    }
    ...
    try {
        ...
        <script>
          ...
          function connectSSE() {
            ...
            eventSource.addEventListener('connected', (event) => {
              ...
              document.getElementById('sse-status').textContent = '已连接 (ID: ' + data.id + ')';
            ...
            eventSource.addEventListener('system', (event) => {
              ...
              document.getElementById('system-status').textContent =
                'CPU: ' + data.cpu + '%, 内存: ' + data.memory + '%, 连接数: ' + data.connections;
            });
      ...
      const sseContainer = `
        <div id="sse-container">
          <h3>SSE状态</h3>
          <p>连接状态: <span id="sse-status">连接中...</span></p>
          <p>系统状态: <span id="system-status">等待数据...</span></p>
          <h4>接收到的事件:</h4>
          <ul id="event-list"></ul>
        </div>
      `;

      template = template.replace('</body>', `${sseContainer}${sseClientScript}</body>`);
...
createServer().then(app => {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`- http://localhost:${PORT}/api/events (General events)`);
    console.log(`- http://localhost:${PORT}/api/stream/system (System status updates)`);
    ...
    setInterval(() => {
      ...
      // 获取所有连接
      if (app.context && app.context.state && app.context.state.sse) {
        ...
        console.log('SSE state not available in app context');

        // 遍历所有请求上下文，查找SSE连接
        let foundConnections = false;
        app.middleware.forEach((middleware, index) => {
          console.log(`Checking middleware ${index}...`);
        });
      }
    }, 10000);
  });
